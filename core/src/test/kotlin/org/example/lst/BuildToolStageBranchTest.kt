...
class BuildToolStageBranchTest {
    ...
    fun `extractClasspath prefers gradlew over gradlew_bat when both exist`() {
    ...
    @Test
    @DisabledOnOs(OS.WINDOWS)
    fun `extractClasspath detects gradlew_bat when only bat wrapper present`() {
        projectDir.resolve("build.gradle").writeText("// empty")
        ...
        assertTrue(result.isSuccess, "Should not throw when gradlew.bat is selected on Linux")
    }
    ...
    @Test
    @DisabledOnOs(OS.WINDOWS)
    fun `extractClasspath returns empty list when gradlew exits 0 with no output`() {
        projectDir.resolve("build.gradle").writeText("// empty")
        val gradlew = projectDir.resolve("gradlew").toFile()
    ...
    @Test
    @DisabledOnOs(OS.WINDOWS)
    fun `extractClasspath with gradlew handles non-zero exit gracefully`() {
        projectDir.resolve("build.gradle").writeText("// empty")
        val gradlew = projectDir.resolve("gradlew").toFile()
        gradlew.writeText("#!/bin/sh\necho some-output\nexit 2\n")
        gradlew.setExecutable(true)
    ...
    @Test
    @DisabledOnOs(OS.WINDOWS)
    fun `extractClasspath returns null when mvnw exits 0 but writes empty output file`() {
    ...
    @Test
    @DisabledOnOs(OS.WINDOWS)
    fun `tryCompile returns true when mvnw exits 0`() {
        projectDir.resolve("pom.xml").writeText("<project/>")
        val mvnw = projectDir.resolve("mvnw").toFile()
        mvnw.writeText("#!/bin/sh\nexit 0\n")
        mvnw.setExecutable(true)
    ...
    @Test
    @DisabledOnOs(OS.WINDOWS)
    fun `tryCompile returns true when gradlew exits 0`() {
        projectDir.resolve("build.gradle").writeText("// empty")
        val gradlew = projectDir.resolve("gradlew").toFile()
        gradlew.writeText("#!/bin/sh\nexit 0\n")
        gradlew.setExecutable(true)
    ...
    @Test
    @DisabledOnOs(OS.WINDOWS)
    fun `tryCompile returns false when gradlew_bat is the only wrapper`() {
        projectDir.resolve("build.gradle").writeText("// empty")
        val gradlewBat = projectDir.resolve("gradlew.bat").toFile()
        gradlewBat.writeText("@echo off\nexit /B 1\n")
        gradlewBat.setExecutable(true)
