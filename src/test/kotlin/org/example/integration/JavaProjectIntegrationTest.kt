...
class JavaProjectIntegrationTest : BaseIntegrationTest() {
    ...
    @Test
    fun `AutoFormat diff preserves class name in output`() {
        projectDir.resolve("Hello.java").writeText(unformattedClass)
    ...
    @Test
    fun `AutoFormat with --dry-run does not write changes to disk`() {
    ...
    @Test
    fun `AutoFormat writes reformatted Java to disk`() {
        val javaFile = projectDir.resolve("Hello.java")
        javaFile.writeText(unformattedClass)
        ...
        assertTrue(formatted.contains("System.out.println"), "Method body must survive formatting")
    }

    // ─── Output modes ─────────────────────────────────────────────────────────

    @Test
    fun `--output files lists changed paths without diff markers`() {
        projectDir.resolve("Hello.java").writeText(unformattedClass)
    ...
    @Test
    fun `--output report creates openrewrite-report json`() {
        projectDir.resolve("Hello.java").writeText(unformattedClass)
    ...
    @Test
    fun `AutoFormat processes all Java files in nested source tree`() {
        val srcDir = projectDir.resolve("src/main/java/com/example").also { it.createDirectories() }
    ...
    @Test
    fun `extension filter limits processing to java files only`() {
        projectDir.resolve("Hello.java").writeText(unformattedClass)
