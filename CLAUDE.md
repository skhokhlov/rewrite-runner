...
- Classpath stages return `null` to fall through and a `ClasspathResolutionResult` to terminate. `LstBuilder` appends project class dirs once after the winning stage. Gradle project data is attached only by a winning Stage 1 or Stage 2 result; Stage 3/4 wins do not inherit metadata from a Stage 2 fall-through. See `docs/adr/0002-classpath-stage-seam.md`.
