 ...
 *    statically to extract `groupId:artifactId:version` coordinates.
 * 2. All coordinates are passed to [DependencyResolutionStage.resolveWithPomTraversal]
 *    for full POM traversal, applying Maven conflict resolution (nearest-wins)
 *    across the full transitive graph.
 *
 ...
 *
 * **Gradle projects:** Delegates to [DependencyResolutionStage.parseGradleDependenciesStatically],
 ...
 * causing [LstBuilder] to fall through to [DirectParseStage] (Stage 4).
 *
 ...
 */
open class BuildFileResolveStage(
    ...
    open fun resolveClasspath(projectDir: Path): List<Path> {
        ...
        return depResolutionStage.resolveWithPomTraversal(coords).also { result ->
    ...
    internal fun gatherAllCoordinates(projectDir: Path): List<String> {
        ...
        if (hasBuildGradle(projectDir)) {
            ...
            coords += depResolutionStage.parseGradleDependenciesStatically(projectDir)
        } else {
    ...
    /**
     * Walks [projectDir] up to depth 3, calling [DependencyResolutionStage.parseGradleDependenciesStatically]
        ...
        try {
            Files.walk(projectDir, 3).use { stream ->
                stream.filter { path ->
                    ...
                    coords += depResolutionStage.parseGradleDependenciesStatically(buildFile.parent)
                }
            }
        } catch (e: Exception) {
    ...
    private fun discoverAndParseMavenPoms(projectDir: Path): List<String> {
        ...
        return pomDirs.flatMap { dir -> depResolutionStage.parseMavenDependencies(dir) }
    }
