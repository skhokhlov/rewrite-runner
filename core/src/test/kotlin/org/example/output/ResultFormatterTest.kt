...
class ResultFormatterTest :
    FunSpec({
        var reportDir: Path = Path.of("")

        beforeEach { reportDir = Files.createTempDirectory("rft-") }

        afterEach { reportDir.toFile().deleteRecursively() }
         ...
         */
        fun makeResult(name: String, before: String, after: String): Result {
            val parser = PlainTextParser()
            val sourceFiles: List<org.openrewrite.SourceFile> =
                parser.parse(ctx, before)
                    .map {
        ...
        fun captureOutput(block: (PrintWriter) -> Unit): String {
        ...
        test("DIFF mode prints unified diff for changed file") {
        ...
        test("DIFF mode prints message when no changes") {
            val output =
                captureOutput { pw ->
        ...
        test("DIFF mode handles multiple results") {
        ...
        test("FILES mode prints only changed file paths") {
            ...
            assertFalse(
                output.contains("---"),
                "FILES mode should not include diff content"
            )
            assertFalse(
                output.contains("+++"),
                "FILES mode should not include diff content"
        ...
        test("FILES mode prints message when no changes") {
            val output =
                captureOutput { pw ->
        ...
        test("FILES mode lists each file on its own line") {
            val results =
                listOf(
                    makeResult("a.txt", "a\n", "aa\n"),
                    makeResult("b.txt", "b\n", "bb\n")
        ...
        test("REPORT mode writes openrewrite-report_json to reportDir") {
            val result = makeResult("Hello.txt", "hello\n", "hello world\n")
            captureOutput { pw ->
        ...
        test("REPORT mode JSON contains changed file path") {
            val result = makeResult("Hello.txt", "hello\n", "hi\n")
            captureOutput { pw ->
                ResultFormatter(OutputMode.REPORT, pw).format(listOf(result), reportDir)
            }
            ...
            assertTrue(
                reportContent.contains("Hello.txt"),
                "Report should contain the file path"
            )
        }

        test("REPORT mode JSON is valid and has expected top-level structure") {
            ...
            assertTrue(
                tree.has("totalChanged"),
                "Report JSON should have 'totalChanged' field"
            )
        ...
        test("REPORT mode JSON result entry has required fields") {
            val result = makeResult("Foo.txt", "foo\n", "bar\n")
            captureOutput { pw ->
                ResultFormatter(OutputMode.REPORT, pw).format(listOf(result), reportDir)
            }
            ...
            assertFalse(
                entry["isNewFile"].asBoolean(),
                "Modified file should not be marked as new"
            )
            assertFalse(
        ...
        test("REPORT mode prints path of generated report file to stdout") {
            val output =
                captureOutput { pw ->
                    ResultFormatter(OutputMode.REPORT, pw).format(emptyList(), reportDir)
                }
            assertTrue(
                output.contains("openrewrite-report.json"),
                "Should print path to report file"
            )
        }

        test("REPORT mode JSON totalChanged is zero for empty results") {
            captureOutput { pw ->
                ResultFormatter(OutputMode.REPORT, pw).format(emptyList(), reportDir)
            }
            val tree =
