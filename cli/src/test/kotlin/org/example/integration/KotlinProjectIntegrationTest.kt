...
class KotlinProjectIntegrationTest : BaseIntegrationTest() {
    ...
    private val unformattedKt =
        """fun greet(name: String): String {return "Hello, ${'$'}name!"}"""

    // ─── Parsing and diff ─────────────────────────────────────────────────────

    @Test
    fun `AutoFormat runs successfully on Kotlin source files`() {
    ...
    @Test
    fun `AutoFormat produces diff or no-changes message for Kotlin source`() {
        projectDir.resolve("Greeter.kt").writeText(unformattedKt)
    ...
    @Test
    fun `FindAndReplace modifies Kotlin source file content`() {
    ...
    @Test
    fun `FindAndReplace with --dry-run does not modify Kotlin file`() {
        val ktFile = projectDir.resolve("Config.kt")
    ...
    @Test
    fun `only kt files are processed when include-extensions is dot-kt`() {
