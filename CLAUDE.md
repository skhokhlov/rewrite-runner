...
- Stage 0 tries the official Gradle/Maven OpenRewrite plugins first and returns `RunResult.rawDiffs` on success. It also forwards data-table export (Maven `-Drewrite.exportDatatables=true`; Gradle `exportDatatables = true`) and reads `ExecutionDiagnostics.estimatedTimeSaved` from the latest exported `SourcesFileResults` table when present, falling back to the plugin's own `Estimate time saved` output line. Use `--skip-plugin-run` / `Builder.skipPluginRun(true)` when testing or debugging the in-process LST pipeline directly.
