...
class MultiModuleJavaVersionTest {
     ...
     */
    private fun buildAndGetVersionsPerFile(): Map<String, JavaVersion> {
    ...
    @Test
    fun `Maven multi-module each submodule gets its own declared Java version`() {
        // Root: Java 11
    ...
    @Test
    fun `Maven multi-module root-level Java files get root pom version`() {
        // Root: Java 21
        projectDir.resolve("pom.xml").writeMavenPom("root", javaVersion = "21")

        val rootSrc = projectDir.resolve("src/main/java").also { it.createDirectories() }
    ...
    @Test
    fun `Maven multi-module submodule with no explicit version inherits root version`() {
    ...
    @Test
    fun `Maven multi-module three level nesting uses nearest ancestor with explicit version`() {
        // Root: Java 11
        projectDir.resolve("pom.xml").writeMavenPom("root", javaVersion = "11")

        // parent-module: no explicit Java version
        val parentMod = projectDir.resolve("parent-module").also { it.createDirectories() }
        ...
        val siblingMod = projectDir.resolve("sibling-module").also { it.createDirectories() }
    ...
    @Test
    fun `Maven multi-module all three submodules get independent versions`() {
        projectDir.resolve("pom.xml").writeMavenPom("root", javaVersion = "21")
    ...
    @Test
    fun `Gradle multi-module subproject without build file walks up to root`() {
    ...
    @Test
    fun `Gradle multi-module files in root src get root build file version`() {
    ...
    @Test
    fun `Gradle multi-module mixed DSL subprojects each get correct version`() {
        projectDir.resolve("build.gradle.kts").writeGradleKtsToolchain(21)
    ...
    @Test
    fun `single-module Maven project behavior is unchanged`() {
        projectDir.resolve("pom.xml").writeMavenPom("single", javaVersion = "17")
        val srcDir = projectDir.resolve("src/main/java").also { it.createDirectories() }
    ...
    @Test
    fun `single-module Gradle project behavior is unchanged`() {
        projectDir.resolve("build.gradle.kts").writeGradleKtsToolchain(21)
        val srcDir = projectDir.resolve("src/main/java").also { it.createDirectories() }
        srcDir.resolve("App.java").writeText("class App {}")
    ...
    @Test
    fun `no build file at all falls back to JVM version for all files`() {
