...
| `--plugin-jvm-args` | | Comma-separated JVM args for the Stage 0 plugin subprocess (e.g. `-Xmx4g`); Gradle gets `-Dorg.gradle.jvmargs` (replaces project value), Maven gets `MAVEN_OPTS` (a project `.mvn/jvm.config` wins). Does not size the runner JVM/LST fallback. | — |
