...
class JavaVersionParsingTest :
    FunSpec({
        ...
        beforeEach { projectDir = Files.createTempDirectory("jvpt-") }
        ...
        fun buildAndInspect(fileName: String): JavaVersion {
            ...
            assertNotNull(
                marker,
                "JavaVersion marker must be attached to parsed Java source file"
            )
            return marker
        }
         ...
         */
        test("Java 8 lambda expressions method references and streams parse correctly") {
            writePom("8")
         ...
         */
        test("Java 11 var local variables and new String API parse correctly") {
            writePom("11")
         ...
         */
        test(
            "Java 17 records sealed interfaces text blocks and instanceof patterns parse correctly"
        ) {
            writePom("17")
         ...
         */
        test(
            "Java 21 switch expressions with type patterns and record patterns parse correctly"
        ) {
            writePom("21")
         ...
         */
        test("Java 25 unnamed patterns and version marker parse correctly") {
            writePom("25")
         ...
         */
        test(
            "AutoFormat recipe does not crash on Java 17 project with records and sealed types"
        ) {
            writePom("17")
         ...
         */
        test(
            "AutoFormat recipe does not crash on Java 21 project with switch patterns"
        ) {
            writePom("21")
         ...
         */
        test(
            "Java 17 project with multiple files in nested directories gets correct version markers"
        ) {
            writePom("17")
            ...
            assertTrue(
                javaFiles.size == 2,
                "Both Java files must be parsed, got: ${javaFiles.map { it.sourcePath }}"
            )
            ...
            for (file in javaFiles) {
                ...
                assertNotNull(
                    marker,
                    "Every parsed Java file must carry a JavaVersion marker"
                )
                assertEquals(
                    "17",
                    marker.sourceCompatibility,
                    "All files in a Java 17 project must get version marker '17', " +
                        "failed for ${file.sourcePath}"
                )
            }
        }
    })
